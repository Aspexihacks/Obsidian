...
> De forma geral podemos representar a energia cinética de um sistema de partículas por$$\begin{align}K&=\sum_{i}K_{i}=\sum_{i} \frac{1}{2}m_{i}(\vec{v}_{i}\cdot \vec{v}_{i})=\sum_{i} \frac{1}{2}m_{i}(\vec{v}_{cm}+\vec{u}_{i})\cdot (\vec{v}_{cm}+\vec{u}_{i})=\\&=\sum_{i} \frac{1}{2}m_{i}v_{cm}^2+\sum_{i} \frac{1}{2}m_{i}u_{i}^2+\sum_{i}m_{i}u{i}^2\end{align}$$